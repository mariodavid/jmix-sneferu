...
@SpringBootTest
class SearchAndSelectByEntityInteractionTest {
  ...
  void given_twoOwnersWithFirstNameA_when_interactionIsPerformed_then_correctEntryIsSelected(UiTestAPI uiTestAPI) {
    ...
    // when:
    openedScreen.interact(searchAndSelect(suggestionField("ownerSuggestionField"), "A", allister));

    // then:
    ownerHasValue(openedScreen, allister);
  }
  ...
  void given_resultsContainsOneEntry_when_selectionEntryNotPresent_then_exceptionIsThrown(UiTestAPI uiTestAPI) {
    ...
    final SuggestionEntryNotFoundException exception = assertThrows(SuggestionEntryNotFoundException.class, () ->
            openedScreen.interact(searchAndSelect(suggestionField("ownerSuggestionField"), "Ash", misty))
    );

    // then:
    assertThat(exception.getMessage())
            .startsWith("Suggestion results did not contain Entity");
  }
